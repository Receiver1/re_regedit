...
HKEY RegEdit::extractKeyHandle(const Key key) {
  switch (key) {
...
RegEdit::RegEdit(const Key key, const std::string &catalog) {
  this->createCatalog(key, catalog);
}

RegEdit::~RegEdit() { this->closeCatalog(); }

void RegEdit::createCatalog(const Key key, const std::string &catalog) {
  if (isOpened_) {
    return;
  }

  const HKEY keyHandle{this->extractKeyHandle(key)};

  const auto result = RegCreateKeyExA(keyHandle, catalog.c_str(), 0, nullptr,
...
void RegEdit::queryValue(const std::string &key,
  ...
  const QueriedValue queriedValue{size, type};
  const void *value{callback(queriedValue)};
  if (!value) {
    return;
  }

  const LPBYTE valueConv{reinterpret_cast<LPBYTE>(value)};
...
void RegEdit::removeValue(const std::string &key) {
  const auto result = RegDeleteValueA(keyHandle_, key.c_str());
  if (result != ERROR_SUCCESS) {
    throw std::system_error{result, std::system_category{}};
  }
}
...
void RegEdit::removeCatalog(Key key, const std::string &catalog) {
  ...
  const auto result = RegDeleteKeyExA(keyHandle, catalog.c_str(), KEY_WOW64_32KEY, 0);
  if (result != ERROR_SUCCESS) {
    throw std::system_error{result, std::system_category{}, __FUNCTION__};
  }
}
...
void RegEdit::closeCatalog() {
  ...
  if (RegCloseKey(keyHandle_) != ERROR_SUCCESS) {
    throw std::system_error{result, std::system_category{}, __FUNCTION__};
  }
